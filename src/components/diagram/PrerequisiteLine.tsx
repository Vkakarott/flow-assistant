...
export function PrerequisiteLine({ from, to, resolved = false }: LineProps) {
    ...
    const startLaneY = from.y + CARD_HEIGHT + GAP_Y / 2;
    const endLaneY = to.y - GAP_Y / 2;
    const rightLaneX = to.x + CARD_WIDTH + GAP_X / 2;
    const leftLaneX = to.x - GAP_X / 2;
    const laneX = fromBottom.x < toTop.x ? leftLaneX : rightLaneX;
    ...
    const directPath = `M ${fromBottom.x} ${fromBottom.y} L ${toTop.x} ${toTop.y}`;
    const path = !goesDown || alignedColumn ? directPath : corridorPath;
